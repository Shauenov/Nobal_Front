...
**Nobal EduConductor** is a Student Admission Management Platform. The frontend described in this document is the **Conductor Dashboard** — a web application used by admission conductors (counselors) to manage their student portfolio: track progress, assign tasks, schedule appointments, send messages, and monitor overall analytics.

### 1.2 User Role

The application is exclusively for users with `role: conductor`. Students have a separate mobile/web interface. All UI decisions, data access, and routing must be scoped to the conductor role.

### 1.3 Design Source

Reference Figma file: `https://www.figma.com/design/V89xlk3ttsGTDx7m3ea661/Nobal-Edu?node-id=0-1`
Focus: Conductor interface panels only. Analyze all frames, auto-layouts, color styles, and text styles before implementation.
...
2. Decode JWT to verify `role === "conductor"`. If role is different, redirect to an "Access Denied" page.
...
All routes under `/(conductor)/*` are wrapped in a `<ProtectedRoute>` component that:
- Checks `isAuthenticated`
- Verifies `user.role === "conductor"`
    ...
    /(conductor)
      ...
      /students
        ...
        /[studentId]
          ...
          /tasks/page.tsx
          /roadmaps/page.tsx
      /tasks/page.tsx            — Global task view (conductor's tasks)
...
3. **Invite Student Button** → opens `InviteModal`:
   ...
   - API: `POST /api/v1/conductor/students/invite`
...
#### Global `/tasks` — Conductor's Personal Tasks

**API**: `POST /api/v1/tasks/personal` for creation; display via student task lists.

> **Note**: There is no dedicated endpoint to list all tasks across all students. **Mock Stub**: Aggregate from student list or add a UI note. Alternatively, implement a conductor-level task view that fetches tasks per student in parallel (up to page 1 of each). Mark this as a **STUB** in code comments.
...
- `title`, `description`, `status`, `priority`, `deadline`, `completed_at`, `is_conductor_task`
...
3. **Manage Slots** — `GET /api/v1/appointments/slots?conductor_id=<me>`

#### Appointment Card fields (`AppointmentOut`):
- `status`: pending / confirmed / completed / cancelled (colored badges)
- `student_id`, `conductor_id` (resolve names via student store or cache)
- `notes`, `cancel_reason`
- `created_at`, `cancelled_at`
- Actions: **Complete** (`PATCH .../complete`), **Cancel** (`PATCH .../cancel` with optional reason)

#### Slot Management:
- **List Slots**: `GET /api/v1/appointments/slots` filtered by `conductor_id` and optionally `from_time`
...
- Each `ConversationOut`: `id`, `student_id`, `conductor_id`, `last_message_at`
...
**UI**:
- Chat bubble layout: conductor messages right-aligned, student messages left-aligned.
...
5. Create appointment slot → student books → conductor completes
...
| 4 | Role guard prevents non-conductor users from accessing the dashboard |
...
*Document prepared for Nobal EduConductor Frontend Project.*
